...
.tab{padding:9px 20px;border-radius:10px;border:1px solid var(--bd);background:var(--c1);color:var(--mu);cursor:pointer;font-size:13px;font-weight:600;transition:all .15s;position:relative;display:flex;align-items:center;gap:8px}
.tab.on{background:linear-gradient(135deg,#1a2a50,#1a1a3a);border-color:#4060c0;color:var(--tx)}
.tab-dot{width:8px;height:8px;border-radius:50%;background:var(--mu);flex-shrink:0;transition:background .4s,box-shadow .4s}
.tab-dot.gr{background:var(--gr);box-shadow:0 0 7px var(--gr)}
.tab-dot.re{background:var(--re);box-shadow:0 0 7px var(--re)}
...
/* ── Events ── */
.ev-card{background:var(--c2);border:1px solid var(--bd);border-radius:10px;padding:16px}
...
.stab{padding:6px 14px;border-radius:8px;border:1px solid var(--bd);background:transparent;color:var(--mu);cursor:pointer;font-size:12px;font-weight:600}
.stab.on{background:var(--bl);border-color:var(--bl);color:#fff}
.spane{display:none}
...
ge('toolsBtn').onclick = function() { showM('toolsModal'); fetchSysDetail(); };
...
function renderTabs(data) {
  ...
  if (!tabsBuilt) {
    ...
    data.targets.forEach(function(t, i) {
      ...
      dot.id = 'tdot-' + i;
      btn.textContent = t.name || ('Server ' + (i + 1));
      btn.appendChild(dot);
...
// ── SVG: Status strip (bar chart of events) ───────────────────────────────
function statusStrip(log) {
  if (!log || !log.trim()) {
    return '<div class="strip-wrap">' +
      Array(20).fill('<div class="strip-bar" style="height:8px;background:var(--c3)"></div>').join('') +
      '</div>';
  }
  ...
  for (var i = 0; i < 20; i++) {
    var line = lines[i] || '';
    var col, h;
    if (!line) { col = 'var(--c3)'; h = 8; }
    else if (line.indexOf('ONLINE') !== -1) { col = 'var(--gr)'; h = 18; }
    else { col = 'var(--re)'; h = 18; }
    bars.push('<div class="strip-bar" style="height:' + h + 'px;background:' + col + ';opacity:' + (0.4 + 0.6 * (i + 1) / 20) + '"></div>');
  }
  return '<div class="strip-wrap">' + bars.join('') + '</div>';
}
...
function openSettings() {
  ...
  spanes.innerHTML = '';
  var panes = [
    { id: 'sp-wifi', label: 'WiFi',     html: buildWifiPane()    },
    { id: 'sp-s0',   label: 'Server 1', html: buildServerPane(0) },
    { id: 'sp-s1',   label: 'Server 2', html: buildServerPane(1) },
    { id: 'sp-s2',   label: 'Server 3', html: buildServerPane(2) }
  ];

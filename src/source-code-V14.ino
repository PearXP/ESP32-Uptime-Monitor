...
// ─── Constants ───────────────────────────────────────────────────────────────
#define CONFIG_VER   14
#define NUM_TARGETS  10
#define SLOG_SIZE    3072
#define TLOG_SIZE    512
#define PING_HIST    20

// RAM warning thresholds (bytes)
#define RAM_WARN_1   (150 * 1024)
#define RAM_WARN_2   (100 * 1024)
#define RAM_WARN_3   ( 50 * 1024)
#define RAM_WARN_4   ( 30 * 1024)
...
// ─── RAM warning tracking ─────────────────────────────────────────────────────
bool ramWarned150 = false;
bool ramWarned100 = false;
bool ramWarned50  = false;
bool ramWarned30  = false;

// ─── Per-server event log (ONLINE/OFFLINE only) ───────────────────────────────
char tgtLog[NUM_TARGETS][TLOG_SIZE] = {};
...
void getDateTime(char* buf, size_t n) {
    struct tm t;
    if (!getLocalTime(&t)) { strncpy(buf, "--.--.-- --:--", n); return; }
    strftime(buf, n, "%d.%m.%y %H:%M:%S", &t);
}

void syslog(const char* fmt, ...) {
    char tb[20], msg[256];
    getDateTime(tb, sizeof(tb));
...
void checkRamWarnings() {
    uint32_t heap = ESP.getFreeHeap();
    if (!ramWarned30  && heap < RAM_WARN_4) { ramWarned30  = true; syslog("⚠ RAM CRITICAL: Free heap below 30 KB (%u B)", heap); }
    else if (!ramWarned50  && heap < RAM_WARN_3) { ramWarned50  = true; syslog("⚠ RAM WARNING: Free heap below 50 KB (%u B)", heap); }
    else if (!ramWarned100 && heap < RAM_WARN_2) { ramWarned100 = true; syslog("⚠ RAM WARNING: Free heap below 100 KB (%u B)", heap); }
    else if (!ramWarned150 && heap < RAM_WARN_1) { ramWarned150 = true; syslog("⚠ RAM NOTICE: Free heap below 150 KB (%u B)", heap); }
    // Reset flags if RAM recovers above a threshold
    if (heap > RAM_WARN_1) { ramWarned150 = false; ramWarned100 = false; ramWarned50 = false; ramWarned30 = false; }
}
...
void checkTarget(int i) {
    ...
    if (ok) {
        ...
        if (!isOnline[i] && sucCnt[i] >= tgt[i].rec_thr) {
            ...
            char tb[20], entry[56]; getDateTime(tb, sizeof(tb));
            snprintf(entry, sizeof(entry), "[%s] ONLINE\n", tb);
            prependLog(tgtLog[i], entry, TLOG_SIZE);
            sendNotifications(i, m.c_str());
            httpGet(tgt[i].http_on);
            syslog("✅ Server %d ONLINE: %s", i + 1, tgt[i].name);
        }
    } else {
        sucCnt[i] = 0;
        failCnt[i]++;
        if (isOnline[i] && failCnt[i] >= tgt[i].fail_thr) {
            ...
            syslog("🚨 Server %d OFFLINE: %s (code %d)", i + 1, tgt[i].name, code);
        }
    }

    // Check RAM warnings after each target check
    checkRamWarnings();
}
...
void setup() {
    ...
    syslog("🔌 Boot: ESP32 Monitor V%d — Reason: %s", CONFIG_VER, reasonStr);
    ...
    syslog("Free heap at boot: %u B (%u KB)", ESP.getFreeHeap(), ESP.getFreeHeap() / 1024);
    ...
    // ── Port Scanner ────────────────────────────────────────────────────────
    srv.on("/api/portscan", HTTP_GET, [](AsyncWebServerRequest* req) {
        if (!req->hasParam("host")) {
            req->send(400, "text/plain", "Missing host parameter");
            return;
        }
        ...
        static const uint16_t ports[] = {
            21, 22, 23, 25, 53, 80, 81, 88, 110, 119, 123, 143, 161, 194,
            443, 445, 465, 500, 514, 515, 587, 631, 993, 995, 1080, 1194,
            1433, 1521, 1723, 1883, 2049, 2082, 2083, 2086, 2087, 2095, 2096,
            3000, 3001, 3306, 3389, 3478, 3690, 4000, 4040, 4433, 4444,
            4500, 5000, 5001, 5005, 5060, 5222, 5432, 5433, 5900, 5984,
            6000, 6379, 6443, 7000, 7070, 7443, 7474, 8000, 8008, 8080,
            8081, 8082, 8086, 8088, 8096, 8123, 8181, 8443, 8444, 8880,
            8888, 8983, 9000, 9001, 9090, 9091, 9200, 9300, 9418, 9443,
            9999, 10000, 27017, 32400, 51413
        };
        ...
        for (int p = 0; p < numPorts; p++) {
            client.setTimeout(1);
            if (client.connect(host.c_str(), ports[p])) {
                client.stop();
                if (result.length() > 0) result += ",";
                result += String(ports[p]);
                openCount++;
            }
        }
        ...
        DynamicJsonDocument doc(2048);
        doc["host"]       = host;
        doc["open_count"] = openCount;
        doc["open_ports"] = result;
        doc["total_scanned"] = numPorts;
        String out; serializeJson(doc, out);
        req->send(200, "application/json", out);
    });
